...
- Image generation uses the Codex subscription image endpoints and fixed `gpt-image-2` defaults. It does not support API keys or silently replay ambiguous failures.
...
- Never retry a streamed request after meaningful output or an image POST after an ambiguous failure.
